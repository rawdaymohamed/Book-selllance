...
class Product(models.Model):
    ...
    # slug: “URL friendly” version of the product or category name
    slug = models.SlugField(max_length=48)
    active = models.BooleanField(default=True)
    in_stock = models.BooleanField(default=True)
    # auto_now --> updates the field with the time the model has been modified
...
class ProductImage(models.Model):
    ...
    thumbnail = ImageField(upload_to='product-thumbnails', null=True)
    ...
    def create_order(self, billing_address, shipping_address):
        ...
        for line in self.basketline_set.all():
            for item in range(line.quantity):
                ...
                c += 1
        logger.info('Created ordr with id=%d and lines_count=%d', order.id, c)
